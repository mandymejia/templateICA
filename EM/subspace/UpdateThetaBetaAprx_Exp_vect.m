function [theta_new, beta_new, z_mode, subICmean, subICvar, grpICmean, grpICvar, error] = UpdateThetaBetaAprx_Exp (Y, X_mtx, theta, C_matrix_diag, beta, N, T, q, p, m, V)
...
for v = 1:V
    ...
    miu_temp = bsxfun(@plus, mtimesx(B,miu3z), Beta_v_trans*X);
    Y_star = bsxfun(@minus, Y_v, mtimesx(A,miu_temp));
    for i = 1:(q+1)
        Sigma23z(:,:,i) = blkdiag(Sigma2, diag(G_z_dict(:,:,i) * theta.sigma3_sq));
    end
    pi_z = mtimesx(G_z_dict,theta.pi);
    mvn_cov = bsxfun(@plus,mtimesx(W2,mtimesx(Sigma23z,W2')),Sigma1);
    Probzvmvn = 10^q * mvnpdf(reshape(permute(Y_star,[3,1,2]),size(Y_star,3),[],1), mvn_mean, mvn_cov);
    
    for i = 1:(q+1)
        Probzv(i) = prod(pi_z(:,:,i);
        Probzv(i) = Probzv(i)*prod(Probzvmvn);
        Sigma_gamma  = eye((N+1)*q)/(Sigma_gamma0 + diag(1./diag(Sigma23z(:,:,i))));
        ...
        miu_sv_svT_all_condz (:, :, i) = miu_star*miu_star'+Sigma_star; 
    end
