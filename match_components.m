function [S_est_match, corr_match] = match_components(S_est, S_true, sign_match)
	% USAGE
	%
	% [S, A, dat_ctr] = dual_reg(dat, S_grp)
	%
    % Matches estimated ICs to true ICs.  
    % Requires the number of estimated ICs to be correct.
    ...
    if(Q ~= size(S_true,2)) error('Different number of ICs in true and estimated S'); end
    ...
    S_est_match = S_est;
    corr_match = diag(corr(S_true, S_est));
