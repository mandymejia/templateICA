...
function [weights,sphere,activations,bias,signs,lrates,y] = icatb_runica_sbica(data,p1,v1,p2,v2,p3,v3,p4,v4,p5,v5,p6,v6,p7,v7,p8,v8,p9,v9,p10,v10,p11,v11,p12,v12,p13,v13,p14,v14)
...
cnt = [0 0 0];
cnt2 = 0;
newlrates = ones(1,size(weights,1));
% avoiding pseudo inverse
%whiteM = pinv(dewhiteM);

while step < maxsteps, %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    permuteVec = randperm(datalength); % shuffle data order at each step

    for t=1:block:lastt, %%%%%%%%% ICA Training Block %%%%%%%%%%%%%%%%%%%
        if biasflag
            u=weights*data(:, permuteVec(t:t+block-1)) + bias*onesrow;
        else
            u=weights*data(:, permuteVec(t:t+block-1));
        end
        if ~extended
    ...
    %%%%%%%%%%%%%%%%%nudging
    %convert to time courses
    cnt2 = cnt2+1;
    %    disp(num2str(cnt2));

    % commenting the equation because it involves inverse of matrix
    %mx = dewhiteM*inv(weights);
    %     % use linsol to solve the equations
    mx = (weights' \ dewhiteM')';

    %%%%%%%
    %mxold = dewhiteM*inv(weights);
    mxold = mx;
    mxdelt = mx-mxold;
    mx = mxold+mxdelt*repmat(newlrates,size(weights,1),1);

    %sort....and reorder if necessary
    for j = 1:size(mx,2);
        D = [(1:length(mx(:,1)))' ones(size(mx(:,1)))];%nuisance signals
        mu2=icatb_ls_fit(D,mx(:,j));        
        tmp = mx(:,j)-D*mu2;%remove nuisance signals
        B2 = icatb_ls_fit(TC,tmp);%fit TC to remainder
        cor(j) = abs(icatb_normdet(icatb_normit2(tmp))'* ...
            icatb_normdet(icatb_normit2(TC*B2)));%compute correlation
    end;
    [Y I] = sort(cor);
    mx = mx(:,I(end:-1:1));%reorder
    ...
    for j = 1:size(mx,2),
        ...
        if (j==1),
            [B2 R2] = icatb_ls_fit(TC,tmp);%fit remainder            
            icaBeta.B2 = B2;
            icaBeta.R2 = R2;
            icaBeta.D = D;
            Dp = std(((icatb_normit2(tmp))-(icatb_normit2(TC*B2))),1);%.*pr(:,j),1);
            Dn = std(((icatb_normit2(tmp))+(icatb_normit2(TC*B2))),1);%.*pr(:,j),1);

            Dp = abs(icatb_normdet(icatb_normit2(tmp))'*icatb_normdet(icatb_normit2(TC*B2)));
            Dn = Dp;
            disp(num2str(Dn));
            icaBeta.rho = Dn;
        else,
        ...
        if D2(j) < prefs(j),
            ...
            if (I==1),
                if (j==1),
                    mx_tmp = tmp/nmx + nweight*(TC*B2/nTC-tmp/nmx);
                else,
                    mx_tmp = tmp/nmx + nweight*(TC*B2/nTC-tmp/nmx);
                end;
            else,
    ...
    %weights=inv(whiteM*mx);
    % avoiding inverse by solving system of equations
    weights = whiteM*mx;
