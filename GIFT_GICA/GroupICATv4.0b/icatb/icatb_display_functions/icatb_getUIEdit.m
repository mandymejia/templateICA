function [editHandle] = icatb_getUIEdit(varargin)
% function used to plot edit text user control on figure
...
% Plot the edit text control on the handle specified
editHandle = uicontrol('parent', Handle, 'tag', Tag, 'string', label, 'units', units, ...
    'position', pos, 'fontunits', UI_FONTUNITS, 'fontname', UI_FONTNAME, 'FontSize', UI_FS, ...
    'style', 'edit', 'ForegroundColor', FONT_COLOR, 'BackgroundColor', BG2_COLOR, 'Visible', visible, ...
    'enable', enable, 'callback', controlCallback, 'fontweight', fontWeight);