function [S,f,Serr]=icatb_mtspectrumc(data,params)
...
[tapers,pad,Fs,fpass,err,trialave,params]=icatb_chronux_getparams(params);
...
data=icatb_change_row_to_column(data);
N=size(data,1);
nfft=max(2^(nextpow2(N)+pad),N);
[f,findx]=icatb_getfgrid(Fs,nfft,fpass); 
tapers=icatb_dpsschk(tapers,N,Fs); % check tapers
J=icatb_mtfftc(data,tapers,nfft,Fs);
J=J(findx,:,:);
S=squeeze(mean(conj(J).*J,2));
if trialave; S=squeeze(mean(S,2));end;
if nargout==3; 
   Serr=icatb_specerr(S,J,err,trialave);
end;
